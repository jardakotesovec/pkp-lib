...
class PKPBladeViewServiceProvider extends ViewServiceProvider
{
    ...
    public function boot()
    {
        ...
        Blade::directive('url', function(string $params) {
            return "<?php
                echo \PKP\\template\\ViewHelper::urlArray($params);
            ?>";
        });

        // Create global aliases so the template helpers can be used without full namespace in templates
        AliasLoader::getInstance()->alias('ViewHelper', \PKP\template\ViewHelper::class);
        AliasLoader::getInstance()->alias('ContentHelper', \PKP\facades\ContentHelper::class);
